...
function net_nehmer_blog_symlink_topic_list()
{
    ...
    mgd_walk_topic_tree("net_nehmer_blog_symlink_topic_list_loop", 0, 99, &$param, true, "name");

    return $param["result"];
}

function net_nehmer_blog_symlink_topic_list_loop ($topicid, $level, &$param)
{
    if ($topicid == 0)
    {
    ...
    if ($topic->parameter("midcom", "component") != "net.nehmer.blog")
    {
        return;
    }
