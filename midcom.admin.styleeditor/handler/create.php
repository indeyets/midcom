...
class midcom_admin_styleeditor_handler_create extends midcom_baseclasses_components_handler
{
    ...
    function _update_breadcrumb()
    {
        ...
        (
            MIDCOM_NAV_URL => "",
            MIDCOM_NAV_NAME => $_MIDCOM->i18n->get_string('create a new element', 'midcom.admin.styleeditor'),
        );
    ...
    function _process_form()
    {
        ...
        if (array_key_exists('midcom_admin_styleeditor_style_name', $_POST))
        {
            if ($_POST['midcom_admin_styleeditor_style_name'] != null || !empty($_FILES))
            {
                ...
                {                                                                                                                                 
                    ...
                    $_MIDCOM->relocate("__mfa/asgard_midcom.admin.styleeditor/edit/".$this->_request_data['style_element_object']->name."/");
                }
                else
                {
        ...
        if ($message)
        {
            $_MIDCOM->uimessages->add($_MIDCOM->i18n->get_string('midcom.admin.styleeditor', 'midcom.admin.styleeditor'), $message);
        }
    }
    ...
    function _handler_create($handler_id, $args, &$data)
    {
        ...
        // Add the page title
        $data['view_title'] = $_MIDCOM->i18n->get_string('create a new element', 'midcom.admin.styleeditor');
