...
class packageMidCOM extends Task
{
    ...
    /**
     * MidCOM component manifest array
     */
    ...
    protected function getFileList($packageInfo)
    {
        ...
        (
            ...
            'prefix' => '    ',
            'baseinstalldir' => 'midcom/lib/' . str_replace('.', '/', $this->package),
            'static' => false,
        );
    ...
    public function getComponentInfo()
    {
        ...
        {
            echo "Note: You have not set packagestate. Defaulting to devel.\n";
            $package['state'] = 'devel';
        }
    ...
    protected function readManifest()
    {
        ...
        {
            echo "Updating manifest to version {$this->version}\n";
            $manifest['version'] = $this->version;
            $this->writeManifest();
        }
        if (   !empty($this->state)
        ...
        {
            echo "Updating manifest to state {$this->state}\n";
            $manifest['state'] = $this->state;
            $this->writeManifest();
        }
    ...
    protected function writeManifest()
    {
        ...
        {
            die("Manifest is not an array, aborting write");
        }
        file_put_contents($file, $this->drawArray($this->manifest));
    }

    protected function drawArray($array, $prefix = '')
    {
        $data = '';
