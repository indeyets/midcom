    ...
    && $config->get('sms_import_enable'))
{
    ...
    if ($_GET['msg'] == 'CANCEL')
    {
        ...
        {
            //echo "Failed to delete log, reason " . mgd_errstr();
            org_routamc_positioning_send_sms($person->handphone, 'Failed to delete log, reason ' . mgd_errstr(), $config->get('smslib_from'), $config);
        }
