 ...
 *
 * This widget should only be used with the blob datatype. It will show a bunch
 * of meta-data fields (unless in simple mode). Deletion is handled through a
 * checkbox while saving the datamanager form.
 *
 * <b>Configuration parameters:</b>
 *
 * <b>widget_blob_simple:</b> Set this to true to reduce the field to a very
 ...
 *
 * The fieldset is set to fieldset.blob. The inputfields are input.shorttext for all
 ...
 * widget is used. The simple widget does not use it. While editing an existing blob,
 * both have the same UI.
 *
 * The download link is a simple paragraph within the enclosing fieldset.
 *
 * The preview link is a nested div: div.blob_preview surrounds the entire preview area,
 * div.image_frame surrounds any displayed image.
 *
 * @package midcom.helper.datamanager
 */
class midcom_helper_datamanager_widget_blob extends midcom_helper_datamanager_widget{
    ...
    function _constructor (&$datamanager, $field, $defaultvalue) {
        ...
        if (! array_key_exists('widget_blob_simple', $field))
        {
            $field['widget_blob_simple'] = false;
        }
        $this->_simple = ($field['widget_blob_simple'] == true);
     ...
     * processing through datatype_blob
     *
     * @see midcom_helper_datamanager_datatype_blob
     */
        ...
        if (is_null($this->_value)) {
            ...
            <table style="padding: 7px; width: 100%; margin-left: 4em;">
             <tr>
              ...
              <td rowspan="3">
            <?php if (substr($this->_value["mimetype"],0,5) == "image") { ?>
    ...
    function draw_widget () {
        if (is_null($this->_value))
        {
            ...
            else
            {
                ...
                $field = $this->_fieldname_mimetype;
                $label = $this->_l10n->get("mime type") . ' (' . $this->_l10n_midcom->get("optional") . ')';
                echo "  <label for='{$field}' id='{$field}_label'>\n";
                echo "    {$label}\n";
                echo "    <input class='shorttext' name='{$field}' id='{$field}' maxlength='255' />\n";
                echo "  </label>\n";
