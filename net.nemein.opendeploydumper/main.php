 ...
 */
class net_nemein_opendeploydumper extends midcom_baseclasses_components_purecode
{
    var $mgd_api = '1.7';
    var $_object = false;
    ...
    function net_nemein_opendeploydumper()
    {
        ...
        $this->_lock_path = $this->_dump_dir . '/net_nemein_opendeploydumper.lock';
    ...
    function deleted($object)
    {
        ...
        if (($state = net_nemein_opendeploydumper_state_dba::get_for($object->guid)) === false)
        {
    ...
    function created($object)
    {
        ...
        if ($ret)
        {
            $stat = net_nemein_opendeploydumper_state_dba::create_for($this->_object, $this->_object_url, 'created');
            if (!$stat)
            {
    ...
    function updated($object)
    {
        ...
        $state = net_nemein_opendeploydumper_state_dba::get_for($this->_object->guid);
        if ($state)
        {
        ...
        if (!$state)
        {
            $stat = net_nemein_opendeploydumper_state_dba::create_for($this->_object, $this->_object_url, 'updated');
            if (!$stat)
            {
                debug_add("Could not create state object for {$this->_object->guid}", MIDCOM_LOG_WARN);
            }
    ...
    function _dump_write_properties()
    {
        ...
        */
        ...
        switch (true)
        {
            case (   $this->mgd_api == '1.7'
                  || is_a($this->_object, 'net_nemein_opendeploydumper_dummyobject')):
        ...
        */
        ...
        if (!is_a($this->_object, 'net_nemein_opendeploydumper_dummyobject'))
        {
