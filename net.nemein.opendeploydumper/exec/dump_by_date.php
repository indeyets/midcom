...
// We need this
class net_nemein_opendeploydumper_dummyobject
{
    var $guid = false;
    var $revised = false;
    var $created = false;
    ...
    function net_nemein_opendeploydumper_dummyobject()
    {
        $this->revised = time();
        $this->created = time();
...
$handler_master = new net_nemein_opendeploydumper();
$handler_master->mode = 'multiple';


echo "<h1>Dump by date</h1>\n";
...
else
{
    ...
    if ($do)
    {
        ...
        foreach ($states as $state)
        {
            debug_add("state\n===\n" . sprint_r($state) . "===\n");
            flush();
            $dummyobject = new net_nemein_opendeploydumper_dummyobject();
