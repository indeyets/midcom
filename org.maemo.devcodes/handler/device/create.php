 ...
 * @package org.maemo.devcodes
 */
class org_maemo_devcodes_handler_device_create extends midcom_baseclasses_components_handler
{
    /**
     * The device which has been created
     *
    ...
    {
        $this->_device = new org_maemo_devcodes_device_dba();
        $this->_device->node = $this->_topic->id;
        $this->_device->codename = $_POST['codename'];

        if (! $this->_device->create())
        {
            debug_push_class(__CLASS__, __FUNCTION__);
            debug_print_r('We operated on this object:', $this->_device);
            debug_pop();
            $_MIDCOM->generate_error(MIDCOM_ERRCRIT, 'Failed to create a new device, cannot continue. Last Midgard error was: '. mgd_errstr());
            // This will exit.
        }
    ...
    function _handler_create($handler_id, $args, &$data)
    {
        ...
        $title = $this->_l10n->get('create device program');
    ...
    function _update_breadcrumb_line()
    {
        ...
        $tmp[] = Array
        (
            MIDCOM_NAV_URL => "create/device.html",
    ...
    {
        midcom_show_style('admin-create-device');
    }
