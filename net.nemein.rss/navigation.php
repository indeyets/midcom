...
/**
 * RSS Aggregator NAP interface class.
 * 
 * @package net.nemein.rss
 */
class net_nemein_rss_navigation {

    var $_object;
    var $_l10n;
    var $_l10n_midcom;

    function net_nemein_rss_navigation() {
    ...
    function get_leaves() {
        ...
        $topic = &$this->_object;
        $leaves = array ();
        if ($articles = mgd_list_topic_articles($topic->id, "reverse score"))
        {
            // Prep toolbar
            $toolbar[50] = Array(
                MIDCOM_TOOLBAR_URL => '',
    ...
    function get_node() {
        ...
        // Hotfix, force a current edited timestamp so that we have regular
        // updates to the Last-Modified timestamp, base it on  the magpie RSS timeout.
        // We base it on the current timestamp, and update it once every MAGPIE_CACHE_AGE
        // multiplies of it.
        $edited = time();
        $edited = (int) ($edited / MAGPIE_CACHE_AGE);
        $edited = $edited * MAGPIE_CACHE_AGE;
