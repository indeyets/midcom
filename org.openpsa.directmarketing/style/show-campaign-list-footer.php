<?php
$view_data =& $_MIDCOM->get_custom_context_data('request_data');
$nap = new midcom_helper_nav();
$node = $nap->get_node($nap->get_current_node());
?>
    </dl>
<?php
if ($view_data['campaigns_all'])
{
    // TODO: Maybe this should be done via AJAX
    ?>
    <form method="post" action="&(_MIDGARD['uri']);">
        <label for="org_openpsa_campaign_selector">
            <?php echo $view_data['l10n']->get('add to campaign'); ?>
            <select name="add_to_campaign" id="org_openpsa_campaign_selector">
                <?php
                foreach ($view_data['campaigns_all'] as $campaign)
                {
                    echo "<option value=\"{$campaign->guid}\">{$campaign->title}</option>\n";
                }
                ?>
            </select>
        </label>
        <input type="submit" value="<?php echo $view_data['l10n']->get('add'); ?>" />
    </form>
    <?php
}
?>
</div>