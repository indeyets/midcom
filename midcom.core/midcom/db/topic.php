...
/** @ignore */
require(MIDCOM_ROOT . '/midcom/baseclasses/database/topic.php');

/**
 * MidCOM Legacy Database Abstraction Layer
 *
 * This class encaspulates a classic MidgardTopic with its original features.
 *
...
class midcom_db_topic extends midcom_baseclasses_database_topic
{
     ...
     */
    function midcom_db_topic($id = null)
    {
        parent::midcom_baseclasses_database_topic($id);
    }
    ...
    function is_in_tree($topic)
    {
        ...
        else if (is_a($topic, 'midcom_baseclasses_database_topic'))
        {
            if ($topic->id == $this->id)
            {
                return true;
