 ...
 * @version $Id:index-legacy-attachments.php 3765 2006-07-31 08:51:39 +0000 (Mon, 31 Jul 2006) tarjei $
...
{
    $attachments = $object->listattachments();
    if (! $attachments)
    {
        debug_add('Failed to query attachments for the object: ' . mgd_errstr());
        return;
    }
    while ($attachments->fetch())
    {
        if (substr($attachments->name, 0, 5) == 'data_')
        {
            debug_print_r("This looks like a DM attachment stored field ('data_' name match), skipping:", $attachments);
            continue;
        }
        
        $attachment = mgd_get_attachment($attachments->id);
        
        // Check only for a truth value, as we are looking only for a set parameter
        if (   $attachment->parameter('midcom.helper.datamanager.datatype.blob', 'fieldname')
            || $attachment->parameter('midcom.helper.datamanager.datatype.image', 'parent_guid'))
        {
