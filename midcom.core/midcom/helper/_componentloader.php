...
class midcom_helper__componentloader
{
    ...
    */
    ...
    /**
     * Returns a reference to an instance of the specified component's
     * COMPONENT class. The component is given in $path as a MidCOM
     * path. Such an instance will be cached by the framework so that
    ...
    function & get_component_class($path)
    {
        ...
        {
            debug_push("midcom_helper__componentloader::get_component_class");
            ...
            debug_add("Instantinating " . $prefix . "_component.", MIDCOM_LOG_DEBUG);
            eval("\$this->_component_classes[\$path] = new " . $prefix . "_component();");
    ...
    /**
     * Returns a reference to an instance of the specified component's
     * Content Admin COMPONENT class. The component is given in $path
     * as a MidCOM path. Such an instance will be cached by the
     * framework so that only one instance is always active for each
     * component. Missing components will be dynamically loaded into
     * memory.
     *
     * @param string $path	The component name.
     * @return mixed		A reference to the concept class in question.
     * @deprecated This has been deprecated in MidCOM 2.4 in favor of the new component interface classes.
     */
    function & get_contentadmin_class($path)
    {
        debug_push_class(__CLASS__, __FUNCTION__);
        debug_add("Warning, this method is deprecated, you should use the new component interface instead.",
            MIDCOM_LOG_DEBUG);
        ...
        if (is_null ($this->_contentadmin_classes[$path]))
        {
            debug_push("midcom_helper__componentloader::get_contentadmin_class");
            ...
            debug_add("Instantinating " . $prefix . "_contentadmin.");
            eval("\$this->_contentadmin_classes[\$path] = new " . $prefix . "_contentadmin();");
        ...
        return $this->_contentadmin_classes[$path];
    }

    /**
     * Returns a reference to an instance of the specified component's
     * NAP class. The component is given in $path as a MidCOM
     * path. Such an instance will be cached by the framework so that
     * only one instance is always active for each component. Missing
     * components will be dynamically loaded into memory.
     *
     ...
     * @deprecated This has been deprecated in MidCOM 2.4 in favor of the new component interface classes.
     */
    function & get_nap_class($path)
    {
        debug_push_class(__CLASS__, __FUNCTION__);
        debug_add("Warning, this method is deprecated, you should use the new component ".
                  "interface instead. (hint: get_interface_class()) ",
            MIDCOM_LOG_DEBUG);
        debug_pop();
        ...
        if (is_null ($this->_nap_classes[$path]))
        {
            debug_push ("midcom_helper__componentloader::get_nap_class");
            ...
            debug_add("Instantinating " . $prefix . "_nap.", MIDCOM_LOG_DEBUG);
            eval("\$this->_nap_classes[\$path] = new " . $prefix . "_nap();");
