 ...
 * 
 */
class midcom_helper_imagepopup_exec_advanced
{
    var $handler = null;

    function midcom_helper_imagepopup_exec_advanced ()
    {
        //$handler =& $_MIDCOM->componentloader->get_component_class('midcom.helper.imagepopup');
        require MIDCOM_ROOT . '/midcom/helper/imagepopup/handler/advanced.php';
        ...
        $this->handler = new midcom_helper_imagepopup_handler_advanced();
    ...
    function run() 
    {
        ...
        if (!$this->handler->_handler_list('list',$GLOBALS['argv']))
        {
            return;
        }
...
$runner = new midcom_helper_imagepopup_exec_advanced();
$runner->run();
