...
$root_topic = mgd_get_object_by_guid($_REQUEST['root_topic_guid']);
...
class net_nemein_wiki_moinmoin_importer
{
    ...
    function import_file($title, $revision_path)
    {
        ...
        */
        ...
        switch (true)
        {
            case (strstr($resolved['remaining_path'], '/')):
                    ...
                    if (!$topic->create())
                    {
                        ...
                        echo "FAILURE: could not create topic, error: " . mgd_errstr() . "<br/>\n";
                        die();
                        return false;
                    }
                    ...
                    */
                    ...
                    else
                    {
                        ...
                        if (!$page->create())
                        {
                            ...
                            echo "FAILURE: Could not create index for new topic, errstr: " . mgd_errstr() . " <br/>\n";
                            $topic->delete();
                            return false;
                        }
                    }
            ...
            default:
                    ...
                    {
                        // TODO: error reporting
                        echo "FAILURE: could not create wikipage object, error: " . mgd_errstr() . "<br/>\n";
                        return false;
                    }
        ...
        {
            // DM2 save failure
            echo "FAILURE: DM2->save() failed, errstr: " . mgd_errstr() . "<br/>\n";
            if (is_object($created_page))
            {
                // Clean up the just created page
                $created_page->delete();
    ...
    function import()
    {
        ...
        while (($page = readdir($dp)) !== false)
        {
            ...
            {
                // Succesfull import of last revision, remove file to avoid double imports
            ...
            switch(true)
            {
                ...
                case (   $char === '|'
                      ...
                      if (preg_match("/<(.*?)=([\"'])?(.*?)\\2?>/", $cell_content, $arbitary_matches))
                      {
                          ...
                          $cell_content = str_replace($arbitary_matches[0], '', $cell_content);
                      }
