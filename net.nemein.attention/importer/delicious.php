 ...
 * @package net.nemein.attention
 */
class net_nemein_attention_importer_delicious extends net_nemein_attention_importer_apml
{
    /**
        ...
        if (count($accounts) > 0)
        {
            foreach ($accounts as $account_param)
            {
                ...
                if (   $user
                    && $user->id)
                {
                    $this->import("http://research.sun.com:8080/AttentionProfile/apml/web/{$account_param->value}", $user->id);
                }
            }
        }
    }
